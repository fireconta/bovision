...
export default function PinLogin() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-4 overflow-hidden relative">
      ...
      <div className="absolute right-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-secondary via-secondary to-transparent opacity-30" />
      ...
      >
        ...
        <div className="relative">
          {/* Outer glow effect */}
          <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-accent/30 via-transparent to-secondary/30 blur-2xl opacity-50" />
          
          {/* Main glass container */}
          <div className="relative bg-background/80 backdrop-blur-xl border-2 border-accent/50 rounded-3xl p-8 shadow-2xl">
            {/* Decorative corner elements */}
            <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2 border-accent opacity-50" />
            <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2 border-secondary opacity-50" />
            <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2 border-accent opacity-50" />
            <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2 border-secondary opacity-50" />
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 my-3 text-accent">
                <div className="w-12 h-0.5 bg-gradient-to-r from-transparent to-accent" />
                <span className="text-sm font-mono font-bold">AI PIN LOGIN</span>
                <div className="w-12 h-0.5 bg-gradient-to-l from-transparent to-accent" />
              </div>

              <p className="text-xs text-muted-foreground">Acesso Seguro • Proteção Inteligente</p>
            </motion.div>
              ...
              >
                ...
                <div className="relative group">
                  ...
                  <button
                    ...
                    className="relative w-full border-2 border-accent/50 rounded-lg p-4 bg-background/50 text-center hover:border-accent transition"
                  >
                    <p className="text-xl font-mono font-bold gradient-text tracking-widest">{deviceId}</p>
            ...
            {(state === 'new_device' || state === 'pin_entry') && (
              ...
              >
                <div className="text-center">
                  <p className="text-xs text-muted-foreground mb-4 font-mono">DIGITE PIN DE 6 DÍGITOS</p>
                  <div className="flex justify-center gap-3 mb-8">
                    {[...Array(6)].map((_, i) => (
                      <motion.div
                        ...
                        initial={{ scale: 0.8 }}
                        animate={{ scale: 1 }}
                        className={`w-10 h-10 rounded-lg border-2 flex items-center justify-center font-bold transition ${
                          i < pin.length
                    ...
                    row.map((digit, j) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`aspect-square rounded-lg border-2 font-bold text-lg transition ${
                            isFingerprint
                ...
                {state === 'pin_entry' && (
                  <Button
                    className="w-full bg-accent hover:bg-accent/90 text-background disabled:opacity-50 font-bold"
                    disabled={pin.length !== 6}
            ...
            {state === 'error' && (
              ...
              >
                <div className="w-16 h-16 rounded-full border-2 border-red-500/50 flex items-center justify-center mx-auto mb-4">
